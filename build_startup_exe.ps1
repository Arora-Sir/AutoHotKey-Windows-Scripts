<#
.SYNOPSIS
    Recompiles StartupScript.exe from AllScripts\StartupScript.ahk with configured icon and base AutoHotkey v2 binary.

.DESCRIPTION
    Task Scheduler launches the compiled StartupScript.exe at user logon.
    Rerun this script whenever StartupScript.ahk source code or included modules change.

.PARAMETER Relaunch
    Stops any running instance, recompiles StartupScript.exe, and triggers Task Scheduler (or direct process start) to relaunch the fleet.

.PARAMETER InstallTask
    Registers the "AHK Startup Script" task in Windows Task Scheduler automatically after compilation.
#>
...
$base     = Join-Path $env:ProgramFiles "AutoHotkey\v2\AutoHotkey64.exe"
...
$compileArgs = @(
    ...
    "/icon", "`"$icon`"",
    "/base", "`"$base`"",
    "/silent", "verbose"   # Required: /silent prevents opening interactive GUI dialog
)
