...
- **The Architecture Solution (`BasicTasks.ahk` + `SharedHelpers.ahk`)**:
  ...
  1. **Target Browser Resolution**: Detects whether Brave or Chrome is active. When actively on Brave or Chrome (via immediate focus, recent focus tracking across tray interactions, or topmost unminimized window in desktop Z-order), the toggle acts strictly on that particular browser instead of parallel-restarting both. If neither browser is active, it inspects running background processes, falling back to Brave as default.
